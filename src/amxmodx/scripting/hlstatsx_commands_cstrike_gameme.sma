...
new g_HLstatsX_block_commands
new g_HLstatsX_message_prefix
...
public plugin_init()
{
	register_plugin("HLstatsX Plugin (gameME)", "2.4+", "TTS Oetzel & Goerz GmbH")
	...
	register_srvcmd("hlx_amx_msay",   "hlx_amx_msay",   0, "<userid><delay><message> - displays advanced information")
	...
	// block origin HLstatsX Stats command setup by default
	...
	TrieSetCell(g_blocked_commands, "/commands", 1)
	TrieSetCell(g_blocked_commands, "!commands", 1)
	TrieSetCell(g_blocked_commands, "hlx_display 0", 1)
	TrieSetCell(g_blocked_commands, "/hlx_display 0", 1)
	TrieSetCell(g_blocked_commands, "!hlx_display 0", 1)
	TrieSetCell(g_blocked_commands, "hlx_display 1", 1)
	TrieSetCell(g_blocked_commands, "/hlx_display 1", 1)
	TrieSetCell(g_blocked_commands, "!hlx_display 1", 1)
/*
	...
	TrieSetCell(g_blocked_commands, "!gameme_atb 1", 1)
*/
	TrieSetCell(g_blocked_commands, "hlx_hideranking", 1)
	TrieSetCell(g_blocked_commands, "/hlx_hideranking", 1)
	TrieSetCell(g_blocked_commands, "!hlx_hideranking", 1)
/*
	...
	TrieSetCell(g_blocked_commands, "!gameme_reset", 1)
*/
	TrieSetCell(g_blocked_commands, "hlx_chat 0", 1)
	TrieSetCell(g_blocked_commands, "/hlx_chat 0", 1)
	TrieSetCell(g_blocked_commands, "!hlx_chat 0", 1)
	TrieSetCell(g_blocked_commands, "hlx_chat 1", 1)
	TrieSetCell(g_blocked_commands, "/hlx_chat 1", 1)
	TrieSetCell(g_blocked_commands, "!hlx_chat 1", 1)
/*
	...
	TrieSetCell(g_blocked_commands, "!global_stats", 1)
*/
	TrieSetCell(g_blocked_commands, "hlx", 1)
	TrieSetCell(g_blocked_commands, "/hlx", 1)
	TrieSetCell(g_blocked_commands, "!hlx", 1)
	TrieSetCell(g_blocked_commands, "hlx_menu", 1)
	TrieSetCell(g_blocked_commands, "/hlx_menu", 1)
	TrieSetCell(g_blocked_commands, "!hlx_menu", 1)
...
*/
	register_cvar("hlxce_version", "2.4+ (HL1)", FCVAR_SPONLY|FCVAR_SERVER)
	register_cvar("hlxce_webpage", "http://www.gameme.com", FCVAR_SPONLY|FCVAR_SERVER)
	g_HLstatsX_block_commands = register_cvar("hlx_block_commands", "1")
	register_srvcmd("hlx_block_commands_values", "hlx_block_commands_values", 0, "<string> - adds blocked chat commands")
	g_HLstatsX_message_prefix = register_cvar("hlx_message_prefix", "")
	...
	menu_additem(g_HLstatsX_MainMenu, "Reset Statistics",       "14")
	menu_additem(g_HLstatsX_MainMenu, "VAC Cheaterlist",        "15")
	menu_additem(g_HLstatsX_MainMenu, "Display Help",           "16")
...
public hlx_block_commands_values(id, level, cid)
{
...
public hlx_amx_psay(id, level, cid)
{
	...
	if (recipients_count > 0) {
		...
		get_pcvar_string(g_HLstatsX_message_prefix, message_prefix, 64)
...
public hlx_amx_browse(id, level, cid)
{
	...
	if ((userid > 0) && (!is_user_bot(userid)) && (is_user_connected(userid))) {
		show_motd(userid, url, "HLstatsX Stats")
	}
...
/*		case 14 : 
			make_player_command(id, "/hlx_reset") */
		case 14 : 
...
public hlx_block_commands(client)
{
	if (client) {
		...
		new block_chat_commands = get_pcvar_num(g_HLstatsX_block_commands)
		...
		if (command_length > 0) {
			if (block_chat_commands > 0) {
				...
				if (is_command_blocked(user_command[start_index]) > 0) {
					if (is_user_connected(client)) {
						if ((strcmp("hlx", user_command[start_index]) == 0) ||
